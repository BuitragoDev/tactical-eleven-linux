...
namespace TacticalEleven.Scripts
{
    public class FinanzasIngresos
    {
        ...
        private Label tituloVentana, gastoSalarioJugadores, dineroTotal, gastoSalarioEmpleados, taquillaNacional, taquillaInternacional,
                      television, ciBonus, abonosClub, ventaJugadores, cesionJugadores, patrocinadorPrincipal, derechosTelevisivos, prestamos, ingresosTotal;
        ...
        public FinanzasIngresos(VisualElement rootInstance, Equipo equipo, Manager manager)
        {
            root = rootInstance;
            miEquipo = equipo;
            miManager = manager;
            ...
            taquillaNacional = root.Q<Label>("taquilla-nacional");
            taquillaInternacional = root.Q<Label>("taquilla-internacional");
            television = root.Q<Label>("television");
            ciBonus = root.Q<Label>("bonus");
            abonosClub = root.Q<Label>("abonos-temporada");
            ventaJugadores = root.Q<Label>("venta-jugadores");
            cesionJugadores = root.Q<Label>("cesion-jugadores");
            patrocinadorPrincipal = root.Q<Label>("patrocinador-principal");
            derechosTelevisivos = root.Q<Label>("derechos-televisivos");
            prestamos = root.Q<Label>("prestamos-bancarios");
            ingresosTotal = root.Q<Label>("total");
        ...
        private void CargarCantidades(int temporada)
        {
            ...
            foreach (Finanza finanza in listaFinanzas)
            {
                ...
                {
                    taquillaInter += (int)finanza.Cantidad;
                }
                if (finanza.IdConcepto == 3)
                {
                    tv += (int)finanza.Cantidad;
                }
            ...
            taquillaNacional.text = Constants.CambioDivisa(taquilla).ToString("N0") + " " + CargarSimboloMoneda();
            taquillaInternacional.text = Constants.CambioDivisa(taquillaInter).ToString("N0") + " " + CargarSimboloMoneda();
            ciBonus.text = Constants.CambioDivisa(bonus).ToString("N0") + " " + CargarSimboloMoneda();
            television.text = Constants.CambioDivisa(tv).ToString("N0") + " " + CargarSimboloMoneda();
            abonosClub.text = Constants.CambioDivisa(abonados).ToString("N0") + " " + CargarSimboloMoneda();
            ventaJugadores.text = Constants.CambioDivisa(ventaJugador).ToString("N0") + " " + CargarSimboloMoneda();
            cesionJugadores.text = Constants.CambioDivisa(cesionJugador).ToString("N0") + " " + CargarSimboloMoneda();
            patrocinadorPrincipal.text = Constants.CambioDivisa(patroPrincipal).ToString("N0") + " " + CargarSimboloMoneda();
            derechosTelevisivos.text = Constants.CambioDivisa(derechosTV).ToString("N0") + " " + CargarSimboloMoneda();
            prestamos.text = Constants.CambioDivisa(prestamoIngreso).ToString("N0") + " " + CargarSimboloMoneda();
            ingresosTotal.text = Constants.CambioDivisa(total).ToString("N0") + " " + CargarSimboloMoneda();
        }
